...
def messages_to_input(data, tokenizer, model, name_of_message=0):
    ...
    if name_of_message==0:
        for target_group, Sentence, Prior_Emotion, _, Possible_Emotions in list(zip(*data)):
        # Compute the correctness score
            messages=[
                    {
                        "role": "user",
                        "content":
                            f"You are a {target_group}. Your prior emotion is {Prior_Emotion} while having the following event:\n"
                            f"{Sentence}\n"
...
def annotate(prediction_set, output_dir,name_of_message):
    ...
    for data in tqdm(prediction_set):
        try:
            input_ids_list,lengths = messages_to_input(data, tokenizer, model, name_of_message)
            terminators = [tokenizer.eos_token_id, tokenizer.convert_tokens_to_ids("<|eot_id|>")]
            input_ids_batch = torch.cat(input_ids_list, dim=0)
            packed_input = pack_padded_sequence(input_ids_batch, lengths, batch_first=True, enforce_sorted=False)
