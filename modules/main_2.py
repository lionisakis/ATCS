...
def parse_args():
    ...
    parser.add_argument("--batch_size", default=64, type=int, help="Number of batch sizes.")
...
class HFModel:
    def __init__(self, name, batch_size, new_tokens, target_group) -> None:
        ...
        # how to add an hf token in the above command?
    ...
    def process(self, data, template):
        ...
        if self.print_message:
            print("A sample message the model took as input is the following:\n",message)
    ...
    def generate_responses(self, dataset, template):
        ...
        dataset = list(map(self.process, dataset.list_of_dicts, [template]*len(dataset)))
        responses = []
        for response in tqdm(
            ...
            print(responses[-1])
        responses = {str(id_): text for id_, text in zip(sentence_id, responses)}
        return responses
...
def main():
    ...
    with open(output_dir +"/"+args.model_name.split("/")[-1]+ "/" + args.target_group + "_" +str(args.template) + "_results.json", "w") as f:
